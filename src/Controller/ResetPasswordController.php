...
class ResetPasswordController extends AbstractController
{
     ...
     *
     * @Route("/{id<[0-9]+>}", name="app_forgot_password_request")
     */
    public function request(?User $user, Request $request, MailerInterface $mailer): Response
    {
        $email = $user->getEmail();
        $form = $this->createForm(ResetPasswordRequestFormType::class, null, [
            'email' => $email,
            'action' => $this->generateUrl('app_forgot_password_request', ['id' => $user->getId()])
        ]);
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer): RedirectResponse
    {
        ...
        $email = (new TemplatedEmail())
            ->from(new Address('no-reply@danstonstock.fr', 'DansTonStock'))
            ->to($user->getEmail())
